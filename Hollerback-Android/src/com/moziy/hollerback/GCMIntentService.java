...
public class GCMIntentService extends GCMBaseIntentService {
    ...
    private static void generateNotification(Context context, String title, String message) {
        ...
        Intent notificationIntent = new Intent(context, HollerbackMainActivity.class);

        // set intent so it does not start a new activity
        notificationIntent.setFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP | Intent.FLAG_ACTIVITY_SINGLE_TOP);
        PendingIntent intent = PendingIntent.getActivity(context, 0, notificationIntent, 0);

        Notification notification = new NotificationCompat.Builder(context).setContentTitle(title).setContentText(message).setContentIntent(intent).setSmallIcon(icon).setWhen(when)
    ...
    private static void generateNotification(Context context, String title, String message, String url) {
        ...
        if ((url == null) || (url.equals(""))) {
            ...
            notificationIntent = new Intent(context, HollerbackMainActivity.class);
        } else {
            // Launch the URL
