...
public class JSONUtil {
    ...
    public static void processVideoPost(JSONObject object, String customMessage) {
        ...
        try {
            ...
            String hash = HashUtil.generateHashFor(IABIntent.ASYNC_REQ_VIDEOS, videoObject.getString("conversation_id"));

            ArrayList<VideoModel> videos = (ArrayList<VideoModel>) QU.getDM().getObjectForToken(hash);
    ...
    public static void processGetConversations(JSONObject object) {
        ...
        try {
            ...
            JSONArray conversationArray = dataObject.getJSONArray("conversations");
            if (conversationArray != null && conversationArray.length() > 0) {

                for (int i = 0; i < conversationArray.length(); i++) {

                    LogUtil.i("Processing JSON " + i);
                    JSONObject conversation = (JSONObject) conversationArray.get(i);
    ...
    public static void processPostConversations(JSONObject json) {
        ...
        try {
            ...
            ArrayList<ConversationModel> conversations = (ArrayList<ConversationModel>) QU.getDM().getObjectForToken(HashUtil.getConvHash());
    ...
    public static void processConversationVideos(long convoId, JSONObject json) {
        try {
            ...
            if (videosArray != null) {
                ...
                String hash = HashUtil.generateHashFor(IABIntent.ASYNC_REQ_VIDEOS, String.valueOf(conversationId));
